...
then
  ...
  aws iam put-role-policy --role-name "$awsCodeBuildRoleName" --policy-name "codebuild-$awsLambdaName-policy" --policy-document "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Sid\":\"Auto0\",\"Effect\":\"Allow\",\"Action\":[\"ecr:GetRegistryPolicy\",\"codedeploy:CreateDeployment\",\"codedeploy:RegisterApplicationRevision\",\"ecr:CreateRepository\",\"ecr:DescribeRegistry\",\"ecr:DescribePullThroughCacheRules\",\"codedeploy:GetDeploymentConfig\",\"ecr:GetAuthorizationToken\",\"ecr:PutRegistryScanningConfiguration\",\"ecr:CreatePullThroughCacheRule\",\"ecr:DeletePullThroughCacheRule\",\"ecr:PutRegistryPolicy\",\"ecr:GetRegistryScanningConfiguration\",\"codebuild:*\",\"ecr:BatchImportUpstreamImage\",\"ecr:DeleteRegistryPolicy\",\"logs:CreateLogStream\",\"logs:CreateLogGroup\",\"ecr:PutReplicationConfiguration\"],\"Resource\":\"*\"},{\"Sid\":\"Auto1\",\"Effect\":\"Allow\",\"Action\":[\"lambda:UpdateFunctionCode\",\"lambda:GetFunction\",\"ecr:*\",\"lambda:PublishVersion\"],\"Resource\":[\"arn:aws:ecr:$awsRegion:$awsAccountID:repository/$awsLambdaName\",\"arn:aws:lambda:$awsRegion:$awsAccountID:function:$awsLambdaName\"]}]}" >/dev/null
  sleep 10
