...
then
  ...
  aws iam put-role-policy --role-name "$awsCodeBuildRoleName" --policy-name "codebuild-$awsLambdaName-policy" --policy-document "{\"Version\":\"2012-10-17\",\"Statement\":[{\"Sid\":\"Auto0\",\"Effect\":\"Allow\",\"Action\":[\"codedeploy:CreateDeployment\",\"codedeploy:RegisterApplicationRevision\",\"codedeploy:GetDeploymentConfig\",\"codebuild:*\",\"ecr:*\",\"logs:*\"],\"Resource\":\"*\"},{\"Sid\":\"Auto1\",\"Effect\":\"Allow\",\"Action\":[\"lambda:UpdateFunctionCode\",\"lambda:GetFunction\",\"lambda:PublishVersion\"],\"Resource\":\"arn:aws:lambda:$awsRegion:$awsAccountID:function:$awsLambdaName\"}]}" >/dev/null
  sleep 10
